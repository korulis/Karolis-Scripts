...
sciMunger = DataMunger(False,2, omitIntercept = False) #must find a bewter name fo this variable 
sciX, sciY = sciMunger.preprocessData('ids_with_their_domains.csv',0,trInt)   
xTest, yTest = getTestXandY(trInt)

print shape(x)
print shape(xTest)

sciNumCols = shape(sciX)[1]
print sciNumCols, 'sciNumCols'
print shape(x), 'shape(x)'

#Trial parameters

thresholds=sort([-1000]+list(np.linspace(-10,10,21))+list(np.linspace(-1,1,21))+[1000])
initTheta = ones(sciNumCols)*0.5
numberOfIterations = 1000*10
regularisationTypes=[1,2]
regularisationCoefficients=[ 100* sqrt(0.1)**(i) for i in range(15) ]
increaseFactor = 2

##
##for regType in regularisationTypes:
##    aucs=[]
##    for regCof in regularisationCoefficients:
##        u=0
##        lgf=LogisticRegressionCostFunction(regularisationCoefficient = regCof, regularisationType = regType)
##        aucs.append(getAUCAndMakeROCFromCrossValidation(lgf,numberOfIterations, increaseFactor , thresholds) )
##    
##    nameString = 'RegCofsAndAUCsAfterCrosVal-WithRegType='+str(regType)
##    aucss=np.array([regularisationCoefficients,aucs])
##np.savetxt(nameString + '.csv', aucss, delimiter=',')

fileName1 = 'RegCofsAndAUCsAfterCrosVal-WithRegType=1.csv'
data1 = genfromtxt(fileName1, delimiter = ',')
regCofs = data1[0][2:]
AUCs1 = data1[1][2:]
##fileName2 = 'RegCofsAndAUCsAfterCrosVal-WithRegType=1.csv'
##data2 = genfromtxt(fileName2, delimiter = ',')
##AUCs2 = data2[1]
AUCs2 = AUCs1+0.003
    
nameString = 'AUCs vs regularisation coefficients for L1 and L2 for All Domains model'
fig = figure()
p1 = fig.add_subplot(1,1,1)
#    c1 = p1.plot(regularisationCoefficients, regularisationCoefficients, 'b-')
c1 = p1.plot(regCofs, AUCs1, 'b-', label = 'L1 regularisation')
c2 = p1.plot(regCofs, AUCs1, 'ro')
c3 = p1.plot(regCofs, AUCs2, 'g-', label = 'L2 regularisation')
c4 = p1.plot(regCofs, AUCs2, 'yo')
p1.set_xscale('log')
p1.set_xlim(0.000001, 200)
p1.grid(True)
p1.legend(loc=3)
xlabel('regularisation coefficient')
ylabel('cross validated AUC')
p1.set_title(nameString)
fig.savefig(nameString+'.png')
show()
close()
...
sys.exit()
lgf=LogisticRegressionCostFunction(regularisationCoefficient = regularisationCoff, regularisationType = 2)
pt= gR(TransposableNPArray(x), y, lgf, descentStepChangingBy(2,2), IterationsStopper(numberOfIterations), initLearningRate, primitiveDescent = False)
karolisTheta10000 = pt.positionVector
fallouts, recalls, precisions = getFalloutsRecallsAndPrecisions(lgf, thresholds, xTest, yTest, karolisTheta10000)
auc = getAUC(fallouts, recalls)
nameString = 'chit'
saveFalloutsAndRecallsToFile(fallouts, recalls, aucstr = nameString)
print auc, 'karolis10000'
plotRecallVSPrecision(recalls, precisions, shouldShow = True , aucstr = nameString)
plotROC(fallouts, recalls, shouldShow = True , aucstr = nameString)
...
###Karolis1000000
##lgf=LogisticRegressionCostFunction(regularisationCoefficient = regularisationCoff, regularisationType = 2)
##pt= gR(TransposableNPArray(x), y, lgf, descentStepChangingBy(2,2), IterationsStopper(1000000), initLearningRate, primitiveDescent = False)
...
##auc = getAUC(fallouts, recalls)
##saveFalloutsAndRecallsToFile(fallouts, recalls, aucstr = 'karolisTrial karolis1000000 auc '+str(auc[0]))
##print auc, 'karolis1000000'
##
##
##
###end of karolis trial
##
##
##
##
...
##
##sys.exit()    
##printTopCoefficientsAbsAndTheirFeatures(10,'temporaryName.csv','theta trInt,regCof,regType,stoppingLimit,increaseFactor0.csv')
##regCof,regType, stoppingLimit, increaseFactor = 0.5,2,100000,2
###thresholds=[-1000,-10,-2,-0.5,0,0.5,2,10,1000]
##thresholds=[-1000,-10,-5,-4,-3,-2,-1,-0.5,-0.25,0,0.25,0.5,1,2,3,4,5,10,1000]
##thresholds=sort([-1000]+list(np.linspace(-10,10,21))+list(np.linspace(-1,1,21))+[1000])
##aucs=getAUCsAndMakeROCFromCrossValidation(regCof,regType, stoppingLimit, increaseFactor, thresholds)
##print aucs
##aucs=np.array(aucs)
...
##sys.exit()    
##def crossValidation():
##    #for regCof,regType, stoppingLimit, increaseFactor in prd([0.1,0.5,1,2],[1,2],[0.1,0.01,0.001,0.001],[1,1.2,2]):
##    #for regCof,regType, stoppingLimit, increaseFactor in prd([0.1,3],[1,2],[100000000],[2]):
##    #loop over parameters
##    for regCof,regType, stoppingLimit, increaseFactor in prd([0.1],[2],[1000],[2]): #dont try to iterate here. WILL BE BAD!!!
...
##            lgf=LogisticRegressionCostFunction(regCof,regType)
##            print 'start gr'
##            pt= gR(theta, TransposableNPArray(x), y, lgf, descentStepChangingBy(increaseFactor,2), IterationsStopper(stoppingLimit,10))
...
##            #print auc, regCof,regType, stoppingLimit, increaseFactor
...
###for regCof,regType, stoppingLimit, increaseFactor in prd([0.1,0.5,1,2],[1,2],[0.1,0.01,0.001,0.001],[1,1.2,2]):
###for regCof,regType, stoppingLimit, increaseFactor in prd([0.1,3],[1,2],[100000000],[2]):
##for regCof,regType, stoppingLimit, increaseFactor in prd([0.1],[2],[100000000],[2]):
##    lgf=LogisticRegressionCostFunction(regCof,regType)
##    pt= gR(theta, TransposableNPArray(x), y, lgf, descentStepChangingBy(increaseFactor,2), IterationsStopper(stoppingLimit,10))
##    result=pt.positionVector
##    for i in range(len(threshold)):
...
##        #print acc, regCof,regType, stoppingLimit, increaseFactor
