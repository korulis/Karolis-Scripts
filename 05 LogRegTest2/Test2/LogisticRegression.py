...
learningRateRule='optimal'
regularisationCoff=0.01
...
clf = linear_model.SGDClassifier(alpha=regularisationCoff, #regularistion coff
                                 ...
                                 learning_rate='optimal', #learning rate change rule
                                 loss='log',
...
clf.n_iter=100
initTheta = ones(sciNumCols)*0.5
clf.fit(sciX, sciY, coef_init=initTheta, intercept_init = 0.5  )
sciIntercept = clf.intercept_
sciTheta100 =  [sciIntercept[0]]+clf.coef_[0].tolist() 
fallouts, recalls = getFalloutsAndRecalls(regularisationCoff, 2, thresholds, xTest, yTest, sciTheta100)
auc = getAUC(fallouts, recalls)
saveFalloutsAndRecallsToFile(fallouts, recalls, aucstr = 'karolisTrial scikit100 auc '+str(auc[0]))
...
clf.fit(sciX, sciY, coef_init=initTheta, intercept_init = 0.5  )
sciIntercept = clf.intercept_
sciTheta10000 =  [sciIntercept[0]]+clf.coef_[0].tolist() 
fallouts, recalls = getFalloutsAndRecalls(regularisationCoff, 2, thresholds, xTest, yTest, sciTheta10000)
auc = getAUC(fallouts, recalls)
saveFalloutsAndRecallsToFile(fallouts, recalls, aucstr = 'karolisTrial scikit10000 auc '+str(auc[0]))
#print loadFalloutsAndRecallsFromFile('FalloutsAndRecallsWith kestsTrial scikit100 auc 0.696332813932.csv')
#plotROC(fallouts, recalls, shouldShow = False , aucstr = 'kestsTrial scikit100 auc '+str(auc[0]))
...
pt= gR(TransposableNPArray(x), y, lgf, descentStepChangingBy(2,2), IterationsStopper(10000), initLearningRate, primitiveDescent = False)
...
auc = getAUC(fallouts, recalls)
saveFalloutsAndRecallsToFile(fallouts, recalls, aucstr = 'karolisTrial karolis10000 auc '+str(auc[0]))
